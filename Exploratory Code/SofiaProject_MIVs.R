...
max(miv_tot$density) # 12000
min(miv_tot$density) # 301.72
plot(yb~xb, type='o', col=low_col, lwd=4,cex=1.5,cex.axis=1, ylim=c(0,12000), xlim=c(140,245), ylab='',xlab='')
mtext(side=2, 'macroinvertebrate density', line=3, cex=1.5)
mtext(side=1, 'Day of Year, 2020', line=3, cex=1.5)
lines(yf~xf, type='o', col =low_col, lwd=4)
lines(yc~xc, type='o', col = high_col, lwd=4)
lines(ye~xe, type='o', col = high_col, lwd=4)
lines(ya~xa, type='o', col = int_col, lwd=4)
lines(yd~xd, type='o', col = int_col, lwd=4)
legend('topright', legend=c('low', 'int', 'high'), col=c(low_col, int_col, high_col), pch=20, cex=1.3)

# Percent change from beginning to end of experiment # 
pchange = miv_tot %>% 
  filter(doy == 154 | doy == 170 | doy == 213 | doy == 226) %>%
  mutate(doy = as.character(doy)) %>%
  mutate(first = case_when(pond_id=='B' & doy == 154 ~ 'first',
                           pond_id=='A' & doy == 154 ~ 'first',
                           pond_id=='C' & doy == 154 ~ 'first', 
                           pond_id=='D' & doy == 154 ~ 'first',
                           pond_id=='E' & doy == 154 ~ 'first',
                           pond_id=='F' & doy == 170 ~ 'first'), 
         last = case_when(pond_id=='B' & doy == 226 ~ 'last',
                          pond_id=='A' & doy == 226 ~ 'last',
                          pond_id=='C' & doy == 213 ~ 'last', 
                          pond_id=='D' & doy == 226 ~ 'last',
                          pond_id=='E' & doy == 226 ~ 'last',
                          pond_id=='F' & doy == 226 ~ 'last'))
pchange
...
miv_tot_comm = hort_mivdensity %>%
  filter(common_name != 'tadpole' & common_name != 'caddisfly_case') %>%
  group_by(pond_id, doy, coupling, feeding_type) %>% 
  summarise(density = sum(density)) %>%
  ungroup() %>% 
  drop_na()
miv_tot_comm
...
miv_comm_wide = miv_tot_comm %>%
  pivot_wider(names_from = 'feeding_type', 
              values_from = 'density')
...
miv_ord_m = as.matrix(miv_comm_wide[,4:8])
...
ordiellipse(miv_nmds, groups = miv_comm_wide$coupling, label=T)

windows(height=4,width=4)
ordiplot(miv_nmds, type='n', xlim=c(-1,1))
...
windows(height=6,width=8) # Dimensions for Ecosphere figure
oo = gg_ordiplot(miv_nmds, groups = miv_comm_wide$coupling, ellipse=T) +
  theme(axis.text.y = element_text(colour = "black", 
                                   size = 12, 
                                   face = "bold"), # y-axis text
        axis.text.x = element_text(colour = "black", 
