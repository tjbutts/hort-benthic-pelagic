...
par(mfrow =c(3,3), mar = c(0,0.5,0.5,0), oma = c(4,4,.5,.5))
par(tcl = -0.25)
par(mgp = c(2, 0.6, 0))
...
boxplot(log(biomass)~treatment, data = low_zoop, ylim = c(log(1), log(800)), 
        ...
        ylab = 'ZP Biomass (ug/L)', col.axis = transparent)
axis(side=2,
...
mtext(side = 2, line = 3.2, 
      expression('Zooplankton'), cex = 9/12)
mtext(side = 2, line = 2, 
...
axis(side=2,
     ...
     las=2)
mtext(side = 2, line = 3.2, 
      expression('Zoobenthos'), cex = 9/12)
mtext(side = 2, line = 2, 
      expression('Density' ~"(#"~L^-1*")"), cex = 9/12)
text(0.55, log(14000), 'D', font = 2)
