...
# Low Coupling Treatment===========
#Total P GAM - Pond A
alg_A_gam2 <- gam(log(chla)~ s(doy, k = 50),data = algA, method = 'REML')
summary(alg_A_gam2) # gam.check(alg_A_gam2)
#Total P GAM - Pond D
alg_D_gam2 <- gam(log(chla)~ s(doy, k = 16),data = algD, method = 'REML')
summary(alg_D_gam2) # gam.check(alg_D_gam2)

# Intermediate Coupling Treatment===========
#Total P GAM - Pond C
alg_C_gam2 <- gam(log(chla)~ s(doy, k = 50),data = algC, method = 'REML')
summary(alg_C_gam2) # gam.check(alg_C_gam2)
#Total P GAM - Pond E
alg_E_gam2 <- gam(log(chla)~ s(doy, k = 50),data = algE, method = 'REML')
summary(alg_E_gam2) # gam.check(alg_E_gam2)

# High Coupling Treatment===========
#Total P GAM - Pond B
alg_B_gam2 <- gam(log(chla)~ s(doy, k = 50),data = algB, method = 'REML')
summary(alg_B_gam2) # gam.check(alg_B_gam2)
#Total P GAM - Pond F
alg_F_gam2 <- gam(log(chla)~ s(doy, k = 50),data = algF, method = 'REML')
